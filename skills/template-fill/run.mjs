...
function parseJsonArrayColumn(value, label) {
...
function parseJsonObjectColumn(value, label) {
  if (value == null || value === "") return {};
  ...
  try {
    ...
    console.error(`[template-fill] WARN: ${label} column contained non-JSON object; treating as empty`);
    return {};
  }
}
...
async function main() {
  ...
  const testimonials = parseJsonArrayColumn(lead.testimonials, "testimonials");
  const gallery = parseJsonArrayColumn(lead.gallery, "gallery");
  const serviceAreaZips = parseJsonArrayColumn(lead.service_area_zips, "service_area_zips");
  const classEBeforeAfter = parseJsonArrayColumn(lead.before_after_photo, "before_after_photo");
  const classEJobPhoto = parseJsonArrayColumn(lead.job_photo, "job_photo");
  ...
  // VAN-60: surface the Class E input envelope directly from demo-gen's
  // fill step so the gate issue can include complete lead evidence fields.
  const classELeadFields = {
    id: lead.id,
    owner_photo: lead.owner_photo || null,
    owner_bio: lead.owner_bio || null,
    before_after_photo: classEBeforeAfter,
    job_photo: classEJobPhoto,
    rating_source: lead.rating_source || null,
    class_e_overrides: parseJsonObjectColumn(lead.class_e_overrides, "class_e_overrides"),
  };
