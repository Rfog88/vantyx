 ...
 *                [--dry-run] [--out <path>]
 *
 *   node run.mjs --lead-json <path> --allow-lead-json --brand-file <path>
 *                --template-dir <path> [...]
 *
 * Exit codes:
...
import {
  CITY_CENTROID_BY_KEY,
  postalCentroid,
  normalizeCityKey,
} from "../brand-consistency-check/centroids.mjs";
...
const NICHE_DEFAULTS = {
  electrician: {
    serviceCopy: "Licensed Electrician",
    emergency24_7: true,
    services: [
...
function deriveHqLatLng(addr) {
  const cityKey = normalizeCityKey(addr.city, addr.state);
  if (!cityKey) {
    die(2, "cannot derive serviceArea.hqLatLng: missing normalized city/state");
  }
  const city = CITY_CENTROID_BY_KEY[cityKey];
  if (!city) {
    die(2, `cannot derive serviceArea.hqLatLng: unsupported city/state centroid for "${addr.city}, ${addr.state}"`);
  }
  if (!String(addr.zip || "").trim()) {
    return [city.lat, city.lng];
  }
  const zipCenter = postalCentroid(addr.zip);
  if (!zipCenter) {
    die(2, `cannot derive serviceArea.hqLatLng: unsupported postal centroid for zip "${addr.zip || ""}"`);
  }
  if (city.lat !== zipCenter.lat || city.lng !== zipCenter.lng) {
    die(2, `cannot derive serviceArea.hqLatLng: city/ZIP centroid mismatch for "${addr.city}, ${addr.state}" and zip "${addr.zip}"`);
  }
  return [zipCenter.lat, zipCenter.lng];
}
...
function heroSubhead(lead) {
  ...
  const source = String(lead.rating_source || "").trim().toLowerCase();
  const sourced = source === "gbp" || source === "google" || source === "google_business_profile";
  if (sourced && Number.isFinite(rating) && rating > 0) {
    return `${rating.toFixed(1)}★ on Google, locally owned, licensed and insured.`;
  }
...
function mapGalleryItem(g) {
  ...
  return {
    ...
    ...(g.city ? { city: g.city } : {}),
    ...(g.placeholderSlot ? { placeholderSlot: g.placeholderSlot } : {}),
  };
}
...
function buildSiteConfig({ lead, brand, enrichment, warnings }) {
  ...
  const hqLatLng = deriveHqLatLng(addr);
  ...
  const normalizeLicense = (value, sourceLabel) => {
    if (typeof value !== "string") return "";
    const trimmed = value.trim();
    if (!trimmed) return "";
    if (/^(x{4,}|0{4,}|9{4,}|-{4,}|placeholder|tbd|unknown)$/i.test(trimmed)) {
      warnings.push(`${sourceLabel} license value "${trimmed}" normalized to canonical placeholder`);
      return "xxxxxxxx";
    }
    return trimmed;
  };
  const baseLicense = normalizeLicense(lead.license_no, "lead");
  const enrichmentLicense = normalizeLicense(en.licenseNumber, "enrichment");
  const licenseNumber = enrichmentLicense || baseLicense;
  const logoUrl = en.logoUrl || "";
  ...
  const companyCity = `${addr.city}, ${addr.state}`;
  const gallery = (enGallery && enGallery.length ? enGallery : dbGallery)
    .map(mapGalleryItem)
    .map((g) => (g.city ? g : { ...g, city: companyCity }));
  const heroImage = en.heroImage || gallery.find((g) => g.src)?.src || "";
  ...
  return {
    ...
    gallery,
    galleryCta: { href: "/work", label: "VIEW MORE PROJECTS" },
    branding: {
...
function main() {
  ...
  if (!args["lead-id"] && !args["lead-json"]) {
    die(1, "missing --lead-id <id>");
  }
  if (args["lead-id"] && args["lead-json"]) {
    die(1, "pass only one lead source: --lead-id OR --lead-json");
  }
  if (args["lead-json"] && !args["allow-lead-json"]) {
    die(
      1,
      "refusing --lead-json without --allow-lead-json (prevents fixture-driven deploys)",
    );
  }
